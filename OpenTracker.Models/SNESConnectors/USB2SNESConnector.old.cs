...
namespace OpenTracker.Models.SNESConnectors
{
    ...
    /// </summary>
    public sealed class USB2SNESConnectorOld : INotifyPropertyChanging, INotifyPropertyChanged, IDisposable
    {
        ...
        /// </param>
        public USB2SNESConnectorOld(string webSocketURI, Action<string, LogLevel> messageHandler = null)
        {
        ...
        {
            Output(string.Format(CultureInfo.InvariantCulture, "USB2SNESConnector is reading a byte: [${0:X6}]",
                address), LogLevel.Info);
            byte[] buffer = new byte[1];

            if (Read(address, buffer))
                return buffer[0];

            Output(string.Format(CultureInfo.InvariantCulture, "USB2SNESConnector failed to read a byte: [${0:X6}]",
                address), LogLevel.Error);
