...
public sealed class SNESConnectorTests
{
    ...
    public async void ConnectAsync_ShouldChangeStatusToSelectDevice_WhenConnectedSuccessfully()
    {
        ...
        _sut.Status.Should().Be(ConnectionStatus.SelectDevice);
    }
    ...
    public async void ConnectAsync_ShouldChangeStatusToError_WhenSocketAlreadyExists()
    {
        ...
        _sut.Status.Should().Be(ConnectionStatus.Error);
    }
    ...
    public async void DisconnectAsync_ShouldChangeStatusToNotConnected()
    {
        ...
        _sut.Status.Should().Be(ConnectionStatus.NotConnected);
    }
    ...
    public async void GetDevicesAsync_ShouldCallProcessResponseAndReturnResultsOnGetDevicesRequest()
    {
        ...
        _getDevicesRequest
            .Received(1)
            .ProcessResponseAndReturnResults(
                Arg.Any<IMessageEventArgsWrapper>(),
                Arg.Any<ManualResetEvent>());
    }
    ...
    public async void AttachDevice_ShouldChangeStatusToConnected_WhenGetDeviceInfoReturnsNotNull()
    {
        _webSocket
            .When(x => x.Connect())
            .Do(_ =>
                _webSocket.OnOpen += Raise.Event<EventHandler>(_webSocket, EventArgs.Empty));
        _getDeviceInfoRequest
            .ProcessResponseAndReturnResults(
                Arg.Any<IMessageEventArgsWrapper>(),
                Arg.Any<ManualResetEvent>())
    ...
    {
        await _sut.ConnectAsync();
        _webSocket.OnClose += Raise
            .Event<EventHandler<CloseEventArgs>>(
                _webSocket,
                FormatterServices.GetUninitializedObject(typeof(CloseEventArgs)));
            
        _logger
            .Received(1)
            .Verbose(Arg.Any<string>());
    }
    ...
    {
        await _sut.ConnectAsync();
        _webSocket.OnError += Raise
            .Event<EventHandler<ErrorEventArgs>>(
                _webSocket,
                FormatterServices.GetUninitializedObject(typeof(ErrorEventArgs)));
            
        _logger
            .Received(1)
            .Verbose(Arg.Any<string>(), Arg.Any<string>());
    }
